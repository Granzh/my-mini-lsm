...
impl MemTable {
    ...
    pub fn scan(&self, _lower: Bound<&[u8]>, _upper: Bound<&[u8]>) -> MemTableIterator {
        let lower_bytes = map_bound(_lower);
        let upper_bytes = map_bound(_upper);
        ...
        let entry = iter.with_iter_mut(|i| i.next().map(|e| (e.key().clone(), e.value().clone())));
...
impl StorageIterator for MemTableIterator {
    ...
    fn next(&mut self) -> Result<()> {
        ...
        if let Some(item) = entry {
            ...
            self.with_item_mut(|i| *i = (bytes::Bytes::new(), bytes::Bytes::new()));
        }
